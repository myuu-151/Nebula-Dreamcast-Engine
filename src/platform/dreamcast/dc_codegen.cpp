    ...
    if (!useLegacyDreamcastBuilder)
    {
        ...
        if (std::filesystem::exists(scriptPath))
        {
            ...
            if (mc.is_open())
            {
                ...
                mc << "}\n";
                mc << "typedef struct { int loaded; int vertCount; int frameCount; float fps; V3* frames; V3* posed; int16_t* remap; int remapCount; int meshAligned; V3 acen; V3 bcen; float ds; int precomputed; } RuntimeAnimClip;\n";
                ...
                mc << "static void runtime_anim_free(RuntimeAnimClip* a){ if(!a) return; if(a->frames){ free(a->frames); a->frames=0; } if(a->posed){ free(a->posed); a->posed=0; } if(a->remap){ free(a->remap); a->remap=0; } a->loaded=0; a->vertCount=0; a->frameCount=0; a->fps=0.0f; a->remapCount=0; a->meshAligned=0; a->precomputed=0; }\n";
