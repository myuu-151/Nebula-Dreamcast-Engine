    ...
    if (!useLegacyDreamcastBuilder)
    {
        ...
        if (std::filesystem::exists(scriptPath))
        {
            ...
            if (mc.is_open())
            {
                ...
                mc << "      int meshAnyOk=0;\n";
                mc << "      for (int i=0;i<rm->kVertCount;++i){ V3 v=(i<srcAnimN)?srcBase[i]:rm->base[i]; v.x*=_scx; v.y*=_scy; v.z*=_scz; float t; ";
                ...
                mc << "v.x+=smPos[0]; v.y+=smPos[1]; v.z+=smPos[2]; cs[i]=w2c(v); ok[i]=proj_cs(cs[i],&sv[i])?1:0; if(ok[i]) meshAnyOk=1; nrm[i]=(V3){0,0,0}; }\n";
