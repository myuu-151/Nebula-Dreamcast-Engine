    ...
    if (!useLegacyDreamcastBuilder)
    {
        ...
        if (std::filesystem::exists(scriptPath))
        {
            ...
            if (mc.is_open())
            {
                ...
                mc << "      }\n";
                mc << "      const V3* srcBase = rm->base; if((sm->animPlaying||sm->animFinished||(sm->runtimeTest)) && _ac->loaded && _ac->frames && _ac->posed && _ac->frameCount>0){ int af; if(sm->animPlaying||sm->animFinished){ af=(int)floorf(sm->animTime*_ac->fps); if(_ac->frameCount>0) af=af%_ac->frameCount; if(af<0) af=0; } else { af=(int)floorf(sRuntimeClock * _ac->fps); if(_ac->frameCount>0) af%=_ac->frameCount; if(af<0) af=0; } if(af!=_ac->lastPosedFrame){ _ac->lastPosedFrame=af; int nv=_ac->vertCount; memcpy(_ac->posed,&_ac->frames[(size_t)af*(size_t)nv],(size_t)nv*sizeof(V3)); for(int vi=nv;vi<rm->kVertCount;++vi) _ac->posed[vi]=rm->base[vi]; } srcBase=_ac->posed; }\n";
                mc << "      float _sx=sinf(rxr),_cx=cosf(rxr),_sy=sinf(ryr),_cy=cosf(ryr),_sz=sinf(rzr),_cz=cosf(rzr);\n";
