                    ...
                    if (!useLegacyDreamcastBuilder)
                    {
                        ...
                        if (std::filesystem::exists(scriptPath))
                        {
                            ...
                            if (mc.is_open())
                            {
                                ...
                                mc << "        }\n";
                                mc << "      }\n";
                                mc << "      /* Accumulate camera-space normals using cached weld groups */\n";
                                mc << "      if (meshShadeMode && rm->weldGroups && rm->nWeldGroups > 0) {\n";
                                mc << "        V3* gn = (V3*)calloc(rm->nWeldGroups, sizeof(V3));\n";
                                mc << "        if (gn) {\n";
                                mc << "          for (int t=0;t<rm->kTriCount;++t){ int ia=tris[t*3+0], ib=tris[t*3+1], ic=tris[t*3+2]; if(ia<0||ib<0||ic<0||ia>=rm->kVertCount||ib>=rm->kVertCount||ic>=rm->kVertCount) continue; V3 a=cs[ia], b=cs[ib], c=cs[ic]; V3 e1=(V3){b.x-a.x,b.y-a.y,b.z-a.z}; V3 e2=(V3){c.x-a.x,c.y-a.y,c.z-a.z}; V3 fn=norm3(cross3(e1,e2)); int ga=rm->weldGroups[ia],gb=rm->weldGroups[ib],gc=rm->weldGroups[ic]; gn[ga].x+=fn.x; gn[ga].y+=fn.y; gn[ga].z+=fn.z; gn[gb].x+=fn.x; gn[gb].y+=fn.y; gn[gb].z+=fn.z; gn[gc].x+=fn.x; gn[gc].y+=fn.y; gn[gc].z+=fn.z; }\n";
