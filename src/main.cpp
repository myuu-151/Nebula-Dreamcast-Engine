...
static void OpenNebMeshInspector(const std::filesystem::path& absMeshPath);
static void OpenNebAnimInspector(const std::filesystem::path& absAnimPath);
static std::filesystem::path ResolveProjectAssetPath(const std::string& relOrAbs);
static void RefreshNebMeshInspectorClipIfNeeded();
static std::filesystem::path GetNebMeshEmbeddedMetaPath(const std::filesystem::path& absMeshPath);
static bool RebuildNebMeshEmbeddedMapping(struct NebMeshInspectorState& st);
static const aiAnimation* FindAnimByNameOrIndex(const aiScene* scene, const std::string& clipName, int clipIndex);
static InspectorMappingQuality ParseMappingQuality(const std::string& q);
static const char* MappingQualityLabel(InspectorMappingQuality q);
static const char* PlaybackModeLabel(InspectorPlaybackMode m);
static bool SampleEmbeddedMergedVerticesCached(
    struct NebMeshInspectorState& st,
...
static NebMeshInspectorState gNebMeshInspectorState;
...
static void DrawNebMeshInspectorWindow(float deltaTime)
{
...
static std::string NormalizeAnimName(const std::string& in)
{
    std::string s;
    s.reserve(in.size());
    for (char c : in)
...
static void CollectSceneNodes(const aiNode* node, std::vector<const aiNode*>& out)
{
    if (!node) return;
    out.push_back(node);
...
static bool ReadS32BE(std::ifstream& in, int32_t& outVal)
{
    uint32_t v;
...
static float FromFixed16_16(int32_t v)
{
...
static void CleanupNebMeshTopology(
    NebMesh& mesh,
    std::vector<uint32_t>* provenanceMeshIndices,
    std::vector<uint32_t>* provenanceVertexIndices)
{
...
static bool ParseUIntCsv(const std::string& s, std::vector<unsigned int>& outValues)
{
    outValues.clear();
...
static bool ParseU32Csv(const std::string& s, std::vector<uint32_t>& outValues)
{
    outValues.clear();
    std::stringstream ss(s);
    std::string token;
...
static void BuildDefaultEmbeddedMetaFromScene(const aiScene* scene, NebMeshEmbeddedAnimMeta& outMeta)
{
...
static bool LoadNebMeshEmbeddedMeta(const std::filesystem::path& absMeshPath, NebMeshEmbeddedAnimMeta& outMeta)
{
...
static uint64_t PackSourceVertexKey(uint32_t meshIndex, uint32_t vertexIndex)
{
    return ((uint64_t)meshIndex << 32ull) | (uint64_t)vertexIndex;
}

static bool BuildMergedSourceIndexTable(
    const aiScene* scene,
...
static bool SampleMergedFbxVertices(
    const aiScene* scene,
    const aiAnimation* anim,
    const std::vector<unsigned int>& meshIndices,
    double timeTicks,
...
static bool ComputeEmbeddedClipDiagnostics(
    const aiScene* scene,
    const aiAnimation* anim,
    const std::vector<unsigned int>& meshIndices,
    float sampleFps,
...
static bool BuildMergedFbxBindData(
    const aiScene* scene,
    const std::vector<unsigned int>& meshIndices,
...
static bool LoadNebAnimClip(const std::filesystem::path& path, NebAnimClip& outClip, std::string& outError)
{
    outClip = NebAnimClip{};
    outError.clear();
...
static bool RebuildNebMeshEmbeddedMapping(NebMeshInspectorState& st)
{
                    ...
                    if (!useLegacyDreamcastBuilder)
                    {
                        ...
                        if (std::filesystem::exists(scriptPath))
                        {
                            ...
                            if (mc.is_open())
                            {
                                ...
                                mc << "typedef struct { char meshDisk[128]; char meshLogical[128]; float pos[3]; float rot[3]; float scale[3]; char texDisk[MAX_SLOT][128]; char texLogical[MAX_SLOT][128]; char animDisk[128]; uint8_t runtimeTest; } SceneMeshMeta;\n";
                                ...
                                mc << "      for (int i=0;i<rm->kVertCount;++i){ V3 v = srcBase[i]; v.x *= sm->scale[0] * (float)gMirrorX; v.y *= sm->scale[1] * (float)gMirrorY; v.z *= sm->scale[2] * (float)gMirrorZ; v = rot_xyz(v, rxr, ryr, rzr); v.x += smPos[0]; v.y += smPos[1]; v.z += smPos[2]; cs[i] = w2c(v); ok[i] = proj_cs(cs[i], &sv[i]) ? 1 : 0; nrm[i]=(V3){0,0,0}; }\n";
                                ...
                                mc << "        rm->nWeldGroups = 0;\n";
                                mc << "        if (rm->weldGroups) {\n";
                                mc << "          const float we2 = (1.0f/512.0f)*(1.0f/512.0f);\n";
                                ...
                                mc << "          }\n";
                                mc << "        }\n";
                                mc << "        rm->baseNormals = (V3*)calloc(rm->kVertCount, sizeof(V3));\n";
                                mc << "        if (rm->weldGroups && rm->baseNormals && rm->nWeldGroups > 0) {\n";
                                mc << "          V3* gn = (V3*)calloc(rm->nWeldGroups, sizeof(V3));\n";
                                mc << "          if (gn) {\n";
                                mc << "            for (int t=0;t<rm->kTriCount;++t){ int ia=rm->trisNormal[t*3+0], ib=rm->trisNormal[t*3+1], ic=rm->trisNormal[t*3+2]; if(ia<0||ib<0||ic<0||ia>=rm->kVertCount||ib>=rm->kVertCount||ic>=rm->kVertCount) continue; V3 a=rm->base[ia], b=rm->base[ib], c=rm->base[ic]; V3 e1=(V3){b.x-a.x,b.y-a.y,b.z-a.z}; V3 e2=(V3){c.x-a.x,c.y-a.y,c.z-a.z}; V3 fn=norm3(cross3(e1,e2)); int ga=rm->weldGroups[ia],gb=rm->weldGroups[ib],gc=rm->weldGroups[ic]; gn[ga].x+=fn.x; gn[ga].y+=fn.y; gn[ga].z+=fn.z; gn[gb].x+=fn.x; gn[gb].y+=fn.y; gn[gb].z+=fn.z; gn[gc].x+=fn.x; gn[gc].y+=fn.y; gn[gc].z+=fn.z; }\n";
                                mc << "            for (int g=0;g<rm->nWeldGroups;++g) gn[g]=norm3(gn[g]);\n";
                                mc << "            for (int i=0;i<rm->kVertCount;++i) rm->baseNormals[i]=gn[rm->weldGroups[i]];\n";
                                mc << "            free(gn);\n";
                                mc << "          }\n";
                                mc << "        }\n";
                                mc << "      }\n";
                                mc << "      /* Use cached normals: rotate precomputed model-space normals to world space */\n";
                                mc << "      if (meshShadeMode && rm->baseNormals) {\n";
                                mc << "        if (srcBase == rm->base) {\n";
                                mc << "          /* Static mesh: just rotate precomputed normals */\n";
                                mc << "          for (int i=0;i<rm->kVertCount;++i) { V3 n=rm->baseNormals[i]; n.x*=(float)gMirrorX; n.y*=(float)gMirrorY; n.z*=(float)gMirrorZ; nrm[i]=norm3(rot_xyz(n, rxr, ryr, rzr)); }\n";
                                mc << "        } else if (rm->weldGroups && rm->nWeldGroups > 0) {\n";
                                mc << "          /* Animated mesh: recompute from animated positions using cached weld groups */\n";
                                mc << "          V3* gn = (V3*)calloc(rm->nWeldGroups, sizeof(V3));\n";
                                mc << "          if (gn) {\n";
                                mc << "            for (int t=0;t<rm->kTriCount;++t){ int ia=rm->trisNormal[t*3+0], ib=rm->trisNormal[t*3+1], ic=rm->trisNormal[t*3+2]; if(ia<0||ib<0||ic<0||ia>=rm->kVertCount||ib>=rm->kVertCount||ic>=rm->kVertCount) continue; V3 a=srcBase[ia], b=srcBase[ib], c=srcBase[ic]; V3 e1=(V3){b.x-a.x,b.y-a.y,b.z-a.z}; V3 e2=(V3){c.x-a.x,c.y-a.y,c.z-a.z}; V3 fn=norm3(cross3(e1,e2)); int ga=rm->weldGroups[ia],gb=rm->weldGroups[ib],gc=rm->weldGroups[ic]; gn[ga].x+=fn.x; gn[ga].y+=fn.y; gn[ga].z+=fn.z; gn[gb].x+=fn.x; gn[gb].y+=fn.y; gn[gb].z+=fn.z; gn[gc].x+=fn.x; gn[gc].y+=fn.y; gn[gc].z+=fn.z; }\n";
                                mc << "            for (int g=0;g<rm->nWeldGroups;++g) gn[g]=norm3(gn[g]);\n";
                                mc << "            for (int i=0;i<rm->kVertCount;++i) { V3 n=gn[rm->weldGroups[i]]; n.x*=(float)gMirrorX; n.y*=(float)gMirrorY; n.z*=(float)gMirrorZ; nrm[i]=norm3(rot_xyz(n, rxr, ryr, rzr)); }\n";
                                mc << "            free(gn);\n";
                                mc << "          }\n";
                                mc << "        }\n";
                                mc << "      }\n";
                                ...
                                mc << "          float nB = dot3(nb,l); if(nB<0.0f)nB=0.0f; float litB=amb + nB*(0.9f-0.25f*sh) - ((nb.y<0.0f)?(-nb.y*0.25f*sh):0.0f) + (1.0f-fabsf(nb.z))*0.12f;\n";
                                mc << "          float nC = dot3(nc,l); if(nC<0.0f)nC=0.0f; float litC=amb + nC*(0.9f-0.25f*sh) - ((nc.y<0.0f)?(-nc.y*0.25f*sh):0.0f) + (1.0f-fabsf(nc.z))*0.12f;\n";
                                mc << "          if(litA<0.10f)litA=0.10f; if(litA>1.20f)litA=1.20f; if(litB<0.10f)litB=0.10f; if(litB>1.20f)litB=1.20f; if(litC<0.10f)litC=0.10f; if(litC>1.20f)litC=1.20f;\n";
                                mc << "          int iA=(int)(litA*255.0f), iB=(int)(litB*255.0f), iC=(int)(litC*255.0f); if(iA<0)iA=0; if(iA>255)iA=255; if(iB<0)iB=0; if(iB>255)iB=255; if(iC<0)iC=0; if(iC>255)iC=255;\n";
