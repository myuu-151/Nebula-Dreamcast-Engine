...
struct Node3DNode
{
    ...
    float velY = 0.0f; // runtime-only quick gravity
};
...
static bool LoadSceneFromPath(const std::filesystem::path& path, SceneData& outScene)
{
    ...
    while (std::getline(in, line))
    {
        ...
        {
            StaticMesh3DNode n;
            ss >> n.name >> n.script >> n.material >> n.mesh
               >> n.x >> n.y >> n.z
               >> n.rotX >> n.rotY >> n.rotZ
               >> n.scaleX >> n.scaleY >> n.scaleZ;
            DecodeSceneToken(n.script);
        ...
        else if (type == "Camera3D")
        {
            ...
            c.x = F(1); c.y = F(2); c.z = F(3);
            c.rotX = F(4); c.rotY = F(5); c.rotZ = F(6);

            // Legacy: name x y z rotX rotY rotZ fov near far main
            // New:    name x y z rotX rotY rotZ perspective fov near far orthoWidth priority main [parent]
            if (toks.size() >= 14)
            {
...
static bool IsLikelySaturnImageBin(const std::filesystem::path& p)
{
    ...
    // Skip obvious non-disc bins from tools/dependencies.
...
static bool GenerateCueForBuild(const std::filesystem::path& buildDir, const std::filesystem::path& projectDir, std::filesystem::path& outCue)
{
    ...
    // Prefer an existing cue if scripts already produced one.
    ...
    // Pick largest likely .bin/.iso as the data track.
                    ...
                    if (!useLegacyDreamcastBuilder)
                    {
                        ...
                        if (std::filesystem::exists(scriptPath))
                        {
                            ...
                            if (mc.is_open())
                            {
                                ...
                                mc << "  V3 tgt = {kMeshPos[0], kMeshPos[1], kMeshPos[2]};\n";
                                ...
                                mc << "      V3 v = base[i];\n";
                                mc << "      v.x *= kMeshScale[0]; v.y *= kMeshScale[1]; v.z *= kMeshScale[2];\n";
                                mc << "      v = rot_xyz(v, deg2rad(kMeshRot[0]), deg2rad(kMeshRot[1]), deg2rad(kMeshRot[2]));\n";
                                mc << "      v.x += kMeshPos[0]; v.y += kMeshPos[1]; v.z += kMeshPos[2];\n";
...

                            {
                                ...
                                if (mk.is_open())
                                {
                                    ...
                                    mk << "CFLAGS += -I$(KOS_BASE)/include -I$(KOS_BASE)/kernel/arch/dreamcast/include -I$(KOS_BASE)/addons/include\n";
