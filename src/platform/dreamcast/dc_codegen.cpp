...
else
{
    ...
    if (!useLegacyDreamcastBuilder)
    {
        ...
        if (std::filesystem::exists(scriptPath))
        {
            ...
            std::vector<std::vector<uint8_t>> runtimeSceneAnimPreloadByMesh;
            ...
            for (const auto& ls : loadedScenes)
            {
                ...
                std::vector<uint8_t> sceneAnimPreloadByMesh;
                ...
                for (const auto& sm : stagedScene.staticMeshes)
                {
                    ...
                    sceneAnimPreloadByMesh.push_back(sm.animPreload ? 1u : 0u);
                ...
                runtimeSceneAnimPreloadByMesh.push_back(std::move(sceneAnimPreloadByMesh));
            ...
            if (runtimeSceneFiles.empty())
            {
                ...
                runtimeSceneAnimPreloadByMesh.push_back({});
            ...
            if (mc.is_open())
            {
                ...
                mc << "#include <math.h>\n";
                mc << "#include <sh4zam/shz_vector.h>\n";
                mc << "#include <sh4zam/shz_matrix.h>\n";
                mc << "#include <sh4zam/shz_xmtrx.h>\n";
                mc << "#include <sh4zam/shz_trig.h>\n";
                mc << "#include <stdio.h>\n";
                mc << "#include <stdlib.h>\n";
                mc << "#include <string.h>\n";
                mc << "#include <stdint.h>\n";
                ...
                mc << "typedef struct { char meshDisk[128]; char meshLogical[128]; char meshName[64]; float pos[3]; float rot[3]; float scale[3]; char texDisk[MAX_SLOT][128]; char texLogical[MAX_SLOT][128]; char animDisk[128]; uint8_t runtimeTest; uint8_t collisionSource; uint8_t animPreload; int animSlotCount; char animSlotName[MAX_ANIM_SLOTS][32]; char animSlotDisk[MAX_ANIM_SLOTS][128]; float animSlotSpeed[MAX_ANIM_SLOTS]; uint8_t animSlotLoop[MAX_ANIM_SLOTS]; int animActiveSlot; int animLoop; float animTime; float animSpeed; int animPlaying; int animFinished; } SceneMeshMeta;\n";
                ...
                mc << "};\n";
                mc << "static const uint8_t kSceneAnimPreload[" << (int)runtimeSceneFiles.size() << "][MAX_MESHES] = {\n";
                for (size_t si = 0; si < runtimeSceneFiles.size(); ++si)
                {
                    mc << "{";
                    for (int mi = 0; mi < 64; ++mi)
                    {
                        int ap = (mi < (int)runtimeSceneAnimPreloadByMesh[si].size()) ? (runtimeSceneAnimPreloadByMesh[si][mi] ? 1 : 0) : 1;
                        mc << ap;
                        if (mi + 1 < 64) mc << ",";
                    }
                    mc << "}";
                ...
                mc << "/* View matrix for sh4zam XMTRX batch transforms (column-major, 8-byte aligned) */\n";
                mc << "static float gViewMat[16] __attribute__((aligned(8)));\n";
                ...
                mc << "  /* Build column-major view matrix for sh4zam */\n";
                mc << "  float tR=-(gBR.x*gBE.x+gBR.y*gBE.y+gBR.z*gBE.z);\n";
                mc << "  float tU=-(gBU.x*gBE.x+gBU.y*gBE.y+gBU.z*gBE.z);\n";
                mc << "  float tF=-(gBF.x*gBE.x+gBF.y*gBE.y+gBF.z*gBE.z);\n";
                mc << "  gViewMat[0]=gBR.x; gViewMat[1]=gBU.x; gViewMat[2]=gBF.x; gViewMat[3]=0;\n";
                mc << "  gViewMat[4]=gBR.y; gViewMat[5]=gBU.y; gViewMat[6]=gBF.y; gViewMat[7]=0;\n";
                mc << "  gViewMat[8]=gBR.z; gViewMat[9]=gBU.z; gViewMat[10]=gBF.z; gViewMat[11]=0;\n";
                mc << "  gViewMat[12]=tR;   gViewMat[13]=tU;   gViewMat[14]=tF;   gViewMat[15]=1;\n";
                mc << "}\n";
                ...
                mc << "}\n";
                mc << "typedef struct { int loaded; int vertCount; int frameCount; float fps; V3* frames; V3* posed; int16_t* remap; int remapCount; int meshAligned; V3 acen; V3 bcen; float ds; int precomputed; int baked; int lastPosedFrame; } RuntimeAnimClip;\n";
                ...
                mc << "static void runtime_anim_free(RuntimeAnimClip* a){ if(!a) return; if(a->frames){ free(a->frames); a->frames=0; } if(a->posed){ free(a->posed); a->posed=0; } if(a->remap){ free(a->remap); a->remap=0; } a->loaded=0; a->vertCount=0; a->frameCount=0; a->fps=0.0f; a->remapCount=0; a->meshAligned=0; a->precomputed=0; a->baked=0; a->lastPosedFrame=-1; }\n";
                ...
                mc << "static void runtime_anim_bake(RuntimeAnimClip* ac, const V3* meshBase, int meshVertCount){\n";
                mc << "  if(!ac||!ac->loaded||!ac->frames||ac->frameCount==0||ac->vertCount==0||ac->meshAligned||ac->baked) return;\n";
                mc << "  int nv=ac->vertCount;\n";
                mc << "  const V3* f0=&ac->frames[0];\n";
                mc << "  V3 bmin=meshBase[0],bmax=meshBase[0],bcen={0,0,0},amin=f0[0],amax=f0[0],acen={0,0,0};\n";
                mc << "  for(int vi=0;vi<nv;++vi){ V3 bp=meshBase[vi],ap=f0[vi]; bcen.x+=bp.x;bcen.y+=bp.y;bcen.z+=bp.z; acen.x+=ap.x;acen.y+=ap.y;acen.z+=ap.z; if(bp.x<bmin.x)bmin.x=bp.x;if(bp.y<bmin.y)bmin.y=bp.y;if(bp.z<bmin.z)bmin.z=bp.z; if(bp.x>bmax.x)bmax.x=bp.x;if(bp.y>bmax.y)bmax.y=bp.y;if(bp.z>bmax.z)bmax.z=bp.z; if(ap.x<amin.x)amin.x=ap.x;if(ap.y<amin.y)amin.y=ap.y;if(ap.z<amin.z)amin.z=ap.z; if(ap.x>amax.x)amax.x=ap.x;if(ap.y>amax.y)amax.y=ap.y;if(ap.z>amax.z)amax.z=ap.z; }\n";
                mc << "  float inv=1.0f/(float)nv; bcen.x*=inv;bcen.y*=inv;bcen.z*=inv; acen.x*=inv;acen.y*=inv;acen.z*=inv;\n";
                mc << "  float bd=sqrtf((bmax.x-bmin.x)*(bmax.x-bmin.x)+(bmax.y-bmin.y)*(bmax.y-bmin.y)+(bmax.z-bmin.z)*(bmax.z-bmin.z));\n";
                ...
                mc << "      if(sm->animPreload){\n";
                mc << "        sm->animActiveSlot=si;\n";
                mc << "      } else {\n";
                mc << "        if(sm->animActiveSlot!=si){\n";
                mc << "          runtime_anim_free(&meshRt[mi].animSlots[0]);\n";
                ...
                mc << "            runtime_anim_load(ap,&meshRt[mi].animSlots[0]);\n";
                mc << "            if(meshRt[mi].animSlots[0].vertCount<meshRt[mi].kVertCount){\n";
                mc << "              V3* np=(V3*)realloc(meshRt[mi].animSlots[0].posed,(size_t)meshRt[mi].kVertCount*sizeof(V3));\n";
                mc << "              if(np){ for(int vi=meshRt[mi].animSlots[0].vertCount;vi<meshRt[mi].kVertCount;++vi) np[vi]=meshRt[mi].base[vi]; meshRt[mi].animSlots[0].posed=np; }\n";
                mc << "            }\n";
                mc << "            runtime_anim_bake(&meshRt[mi].animSlots[0],meshRt[mi].base,meshRt[mi].kVertCount);\n";
                mc << "          }\n";
                mc << "          sm->animActiveSlot=si;\n";
                mc << "        }\n";
                mc << "      }\n";
                ...
                mc << "  for(int mi=0; mi<gSceneMeshCount && mi<MAX_MESHES; ++mi){ RuntimeSceneMesh* rm=&meshRt[mi]; SceneMeshMeta* sm=&gSceneMeshes[mi]; if(!rm->loaded) continue; if(!sm->runtimeTest) continue; if(sm->animPreload){ for(int asi=0;asi<sm->animSlotCount&&asi<MAX_ANIM_SLOTS;++asi){ if(!sm->animSlotDisk[asi][0]) continue; char ap[256]; snprintf(ap,sizeof(ap),\"/cd/data/animations/%s\",sm->animSlotDisk[asi]); { char resolved[512]; FILE* fp=dc_fopen_iso_compat(ap,resolved,(int)sizeof(resolved)); if(fp){ fclose(fp); strncpy(ap,resolved,sizeof(ap)-1); ap[sizeof(ap)-1]=0; } } dbgio_printf(\"[NEBULA][DC][ANIM] mesh[%d] preload slot[%d]=%s %s\\n\",mi,asi,sm->animSlotName[asi],ap); if(!runtime_anim_load(ap,&rm->animSlots[asi])){ runtime_anim_free(&rm->animSlots[asi]); continue; } if(rm->animSlots[asi].vertCount>rm->kVertCount){ runtime_anim_free(&rm->animSlots[asi]); continue; } if(rm->animSlots[asi].vertCount<rm->kVertCount){ V3* np=(V3*)realloc(rm->animSlots[asi].posed,(size_t)rm->kVertCount*sizeof(V3)); if(!np){ runtime_anim_free(&rm->animSlots[asi]); continue; } for(int vi=rm->animSlots[asi].vertCount;vi<rm->kVertCount;++vi) np[vi]=rm->base[vi]; rm->animSlots[asi].posed=np; } runtime_anim_bake(&rm->animSlots[asi],rm->base,rm->kVertCount); dbgio_printf(\"[NEBULA][DC][ANIM] mesh[%d] slot[%d]=%s loaded frames=%d fps=%.2f baked=%d\\n\",mi,asi,sm->animSlotName[asi],rm->animSlots[asi].frameCount,rm->animSlots[asi].fps,rm->animSlots[asi].baked); } } else { if(sm->animDisk[0]){ char ap[256]; snprintf(ap,sizeof(ap),\"/cd/data/animations/%s\",sm->animDisk); { char resolved[512]; FILE* fp=dc_fopen_iso_compat(ap,resolved,(int)sizeof(resolved)); if(fp){ fclose(fp); strncpy(ap,resolved,sizeof(ap)-1); ap[sizeof(ap)-1]=0; } } dbgio_printf(\"[NEBULA][DC][ANIM] mesh[%d] disc-io load default %s\\n\",mi,ap); if(runtime_anim_load(ap,&rm->animSlots[0])){ if(rm->animSlots[0].vertCount>rm->kVertCount){ runtime_anim_free(&rm->animSlots[0]); } else { if(rm->animSlots[0].vertCount<rm->kVertCount){ V3* np=(V3*)realloc(rm->animSlots[0].posed,(size_t)rm->kVertCount*sizeof(V3)); if(!np){ runtime_anim_free(&rm->animSlots[0]); } else { for(int vi=rm->animSlots[0].vertCount;vi<rm->kVertCount;++vi) np[vi]=rm->base[vi]; rm->animSlots[0].posed=np; } } runtime_anim_bake(&rm->animSlots[0],rm->base,rm->kVertCount); } } } } }\n";
                ...
                mc << "      int _asi = sm->animPreload ? ((sm->animActiveSlot>=0 && sm->animActiveSlot<MAX_ANIM_SLOTS) ? sm->animActiveSlot : 0) : 0;\n";
                mc << "      RuntimeAnimClip* _ac = &rm->animSlots[_asi];\n";
                mc << "      if(sm->animPlaying && _ac->loaded && _ac->frames && _ac->frameCount>0){\n";
                mc << "        sm->animTime += dt * sm->animSpeed;\n";
                mc << "        int af=(int)floorf(sm->animTime * _ac->fps);\n";
                mc << "        if(af>=(int)_ac->frameCount){ sm->animFinished=1; if(sm->animLoop){ float dur=(float)_ac->frameCount/_ac->fps; if(dur>0.0f){ while(sm->animTime>=dur) sm->animTime-=dur; } else { sm->animTime=0.0f; } } else { sm->animPlaying=0; sm->animTime=(float)(_ac->frameCount-1)/_ac->fps; } }\n";
                mc << "      }\n";
                mc << "      const V3* srcBase = rm->base; int srcAnimN=rm->kVertCount; if((sm->animPlaying||sm->animFinished||(sm->runtimeTest)) && _ac->loaded && _ac->frames && _ac->frameCount>0){ int af; if(sm->animPlaying||sm->animFinished){ af=(int)floorf(sm->animTime*_ac->fps); if(_ac->frameCount>0) af=af%_ac->frameCount; if(af<0) af=0; } else { af=(int)floorf(sRuntimeClock * _ac->fps); if(_ac->frameCount>0) af%=_ac->frameCount; if(af<0) af=0; } srcBase=&_ac->frames[(size_t)af*(size_t)_ac->vertCount]; srcAnimN=_ac->vertCount; }\n";
                mc << "      float _scx=sm->scale[0]*(float)gMirrorX, _scy=sm->scale[1]*(float)gMirrorY, _scz=sm->scale[2]*(float)gMirrorZ;\n";
                mc << "      /* Build model-view matrix using sh4zam XMTRX (SH4 ftrv ~4 cycles/vertex) */\n";
                mc << "      shz_xmtrx_init_translation(smPos[0], smPos[1], smPos[2]);\n";
                mc << "      if(useQuat){ float qm[16]={qRm[0],qRm[3],qRm[6],0, qRm[1],qRm[4],qRm[7],0, qRm[2],qRm[5],qRm[8],0, 0,0,0,1}; shz_xmtrx_apply_unaligned_4x4(qm); }\n";
                mc << "      else { shz_xmtrx_rotate_x(rxr); shz_xmtrx_rotate_y(ryr); shz_xmtrx_rotate_z(rzr); }\n";
                mc << "      if(cRx!=0.0f||cRy!=0.0f||cRz!=0.0f){ shz_xmtrx_rotate_x(cRx); shz_xmtrx_rotate_y(cRy); shz_xmtrx_rotate_z(cRz); }\n";
                mc << "      shz_xmtrx_scale(_scx, _scy, _scz);\n";
                mc << "      shz_xmtrx_apply_reverse_unaligned_4x4(gViewMat); /* XMTRX = View * Model */\n";
                mc << "      int meshAnyOk=0;\n";
                mc << "      for (int i=0;i<rm->kVertCount;++i){ V3 v=(i<srcAnimN)?srcBase[i]:rm->base[i]; shz_vec3_t _pt; _pt.x=v.x; _pt.y=v.y; _pt.z=v.z; shz_vec3_t _cp=shz_xmtrx_transform_point3(_pt); cs[i]=(V3){_cp.x,_cp.y,_cp.z}; ok[i]=proj_cs(cs[i],&sv[i])?1:0; if(ok[i]) meshAnyOk=1; nrm[i]=(V3){0,0,0}; }\n";
                mc << "      if(!meshAnyOk) continue;\n";
                ...
                mc << "            draw_tri_vc(&rm->hdrSlot[sid],sa,sb,sc,colA,colB,colC); continue;\n";
                mc << "          }\n";
                mc << "        }\n";
                mc << "        /* Trivial reject: all 3 verts behind camera — skip before expensive clipper */\n";
                mc << "        if(cs[ia].z<kClipNear && cs[ib].z<kClipNear && cs[ic].z<kClipNear) continue;\n";
                ...
                mc << "          draw_tri_vc(&rm->hdrSlot[sid],svp[0],svp[i],svp[i+1],ca,cb,cc);\n";
                mc << "        }\n";
                mc << "      }\n";
                mc << "    }\n";
                mc << "\n";
