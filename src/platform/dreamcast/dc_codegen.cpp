    ...
    if (!useLegacyDreamcastBuilder)
    {
        ...
        if (std::filesystem::exists(scriptPath))
        {
            ...
            if (mc.is_open())
            {
                ...
                mc << "}\n";
                mc << "typedef struct { int loaded; int vertCount; int frameCount; float fps; V3* frames; V3* posed; int16_t* remap; int remapCount; int meshAligned; int baked; int lastPosedFrame; } RuntimeAnimClip;\n";
                ...
                mc << "static void runtime_anim_free(RuntimeAnimClip* a){ if(!a) return; if(a->frames){ free(a->frames); a->frames=0; } if(a->posed){ free(a->posed); a->posed=0; } if(a->remap){ free(a->remap); a->remap=0; } a->loaded=0; a->vertCount=0; a->frameCount=0; a->fps=0.0f; a->remapCount=0; a->meshAligned=0; a->baked=0; a->lastPosedFrame=-1; }\n";
                ...
                mc << "  if(!ac||!ac->loaded||!ac->frames||ac->frameCount==0||ac->vertCount==0||ac->baked) return;\n";
                mc << "  int nv=ac->vertCount;\n";
                mc << "  /* Compute mesh bounding box + centroid */\n";
                mc << "  V3 bmin={meshBase[0].x,meshBase[0].y,meshBase[0].z}, bmax=bmin;\n";
                mc << "  for(int i=1;i<meshVertCount;++i){ if(meshBase[i].x<bmin.x) bmin.x=meshBase[i].x; if(meshBase[i].y<bmin.y) bmin.y=meshBase[i].y; if(meshBase[i].z<bmin.z) bmin.z=meshBase[i].z; if(meshBase[i].x>bmax.x) bmax.x=meshBase[i].x; if(meshBase[i].y>bmax.y) bmax.y=meshBase[i].y; if(meshBase[i].z>bmax.z) bmax.z=meshBase[i].z; }\n";
                mc << "  V3 bcen={(bmin.x+bmax.x)*0.5f,(bmin.y+bmax.y)*0.5f,(bmin.z+bmax.z)*0.5f};\n";
                mc << "  float bdx=bmax.x-bmin.x, bdy=bmax.y-bmin.y, bdz=bmax.z-bmin.z;\n";
                mc << "  float bd=sqrtf(bdx*bdx+bdy*bdy+bdz*bdz);\n";
                mc << "  /* Compute anim frame-0 bounding box + centroid */\n";
                mc << "  V3* f0=ac->frames;\n";
                mc << "  V3 amin={f0[0].x,f0[0].y,f0[0].z}, amax=amin;\n";
                mc << "  for(int i=1;i<nv;++i){ if(f0[i].x<amin.x) amin.x=f0[i].x; if(f0[i].y<amin.y) amin.y=f0[i].y; if(f0[i].z<amin.z) amin.z=f0[i].z; if(f0[i].x>amax.x) amax.x=f0[i].x; if(f0[i].y>amax.y) amax.y=f0[i].y; if(f0[i].z>amax.z) amax.z=f0[i].z; }\n";
                mc << "  V3 acen={(amin.x+amax.x)*0.5f,(amin.y+amax.y)*0.5f,(amin.z+amax.z)*0.5f};\n";
                mc << "  float adx=amax.x-amin.x, ady=amax.y-amin.y, adz=amax.z-amin.z;\n";
                mc << "  float ad=sqrtf(adx*adx+ady*ady+adz*adz);\n";
                mc << "  float ds=(ad>1e-6f&&bd>1e-6f)?(bd/ad):1.0f;\n";
                mc << "  /* Skip bake if scale ratio is close to 1.0 and centroids match */\n";
                mc << "  float cdx=fabsf(bcen.x-acen.x), cdy=fabsf(bcen.y-acen.y), cdz=fabsf(bcen.z-acen.z);\n";
                mc << "  if(fabsf(ds-1.0f)<0.01f && cdx<0.1f && cdy<0.1f && cdz<0.1f){ ac->baked=1; ac->lastPosedFrame=-1; return; }\n";
                mc << "  /* Bake centroid+scale into all frames */\n";
                ...
                mc << "  ac->baked=1; ac->lastPosedFrame=-1;\n";
                mc << "  dbgio_printf(\"[NEBULA][DC][ANIM] bake: ds=%.4f acen=(%.3f,%.3f,%.3f) bcen=(%.3f,%.3f,%.3f)\\n\",ds,acen.x,acen.y,acen.z,bcen.x,bcen.y,bcen.z);\n";
                mc << "}\n";
                mc << "\n";
                ...
                mc << "      if(_trPass==0 && sm->animPlaying && _ac->loaded && _ac->frames && _ac->frameCount>0){\n";
                ...
                mc << "      }\n";
                mc << "      const V3* srcBase = rm->base; if((sm->animPlaying||sm->animFinished||(sm->runtimeTest)) && _ac->loaded && _ac->frames && _ac->posed && _ac->frameCount>0){ int af; if(sm->animPlaying||sm->animFinished){ af=(int)floorf(sm->animTime*_ac->fps); if(_ac->frameCount>0) af=af%_ac->frameCount; if(af<0) af=0; } else { float rtSpd=sm->animSlotSpeed[0]; if(rtSpd<=0.001f) rtSpd=1.0f; af=(int)floorf(sRuntimeClock * rtSpd * _ac->fps); if(_ac->frameCount>0) af%=_ac->frameCount; if(af<0) af=0; } const V3* fr=&_ac->frames[(size_t)af*(size_t)_ac->vertCount]; int nv=_ac->vertCount; memcpy(_ac->posed,fr,(size_t)nv*sizeof(V3)); for(int vi=nv;vi<rm->kVertCount;++vi) _ac->posed[vi]=rm->base[vi]; srcBase=_ac->posed; }\n";
                mc << "      float _sx=sinf(rxr),_cx=cosf(rxr),_sy=sinf(ryr),_cy=cosf(ryr),_sz=sinf(rzr),_cz=cosf(rzr);\n";
