...
Mat4 BuildCamera3DProjectionMatrix(const Camera3DProjection& proj)
{
    ...
    else
    {
        ...
        r = Mat4Identity();
        r.m[0] = 2.0f / (proj.orthoWidth - -proj.orthoWidth);
        r.m[5] = 2.0f / (orthoHeight - -orthoHeight);
        r.m[10] = -2.0f / (proj.farZ - proj.nearZ);
        r.m[12] = -(proj.orthoWidth + -proj.orthoWidth) / (proj.orthoWidth - -proj.orthoWidth);
        r.m[13] = -(orthoHeight + -orthoHeight) / (orthoHeight - -orthoHeight);
...
// ---------------------------------------------------------------------------
// NebulaCamera3D functions (runtime / Dreamcast export)
// ---------------------------------------------------------------------------
