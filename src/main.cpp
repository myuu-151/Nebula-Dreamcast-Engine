...
int main(int, char**)
{
    ...
    float viewYaw = -136.1f;
    float viewPitch = -14.2f;
    float distance = 10.0f;
    Vec3 orbitCenter = { 14.812f, 7.632f, 14.834f };
                    ...
                    if (!useLegacyDreamcastBuilder)
                    {
                        ...
                        if (std::filesystem::exists(scriptPath))
                        {
                            ...
                            if (mc.is_open())
                            {
                                ...
                                mc << "      if (NB_KOS_ButtonPressed(NB_BTN_DPAD_UP))    { NB_SetMirrorFromIndex(0); dbgio_printf(\"[Mirror] UP: normal (X=1 Y=1 Z=1)\\n\"); }\n";
