            ...
            auto getMatState = [&](int matIndex) -> MatState {
                ...
                if (!matRef.empty())
                {
                    ...
                    else
                    {
                        ...
                        { float su=1,sv=1,ou=0,ov=0,rd=0; LoadMaterialUvTransform(matPath,su,sv,ou,ov,rd); st.uvScaleU=su; st.uvScaleV=sv; }
                    ...
                    if (!useLegacyDreamcastBuilder)
                    {
                        ...
                        if (std::filesystem::exists(scriptPath))
                        {
                                ...
                                if (!stagingNameCollision)
                                {
                                    ...
                                    for (const auto& key : sortedTexAbs)
                                    {
                                        ...
                                        // Append extension chunk (filter, wrap, flipU, flipV) from .nebtex.meta
                                        {
                                            ...
                                            int extFilter = LoadNebTexFilterMode(srcTex);
                                            int extWrap = LoadNebTexWrapMode(srcTex);
                                            bool extFlipU = false, extFlipV = false;
                                            LoadNebTexFlipOptions(srcTex, extFlipU, extFlipV);
                            ...
                            if (mc.is_open())
                            {
                                ...
                                mc << "  SV* gSv=NULL; uint8_t* gOk=NULL; V3* gCs=NULL; V3* gNrm=NULL; V3* gWs=NULL;\n";
                                mc << "  if(maxVertCount>0){ gSv=(SV*)malloc((size_t)maxVertCount*sizeof(SV)); gOk=(uint8_t*)malloc((size_t)maxVertCount); gCs=(V3*)malloc((size_t)maxVertCount*sizeof(V3)); gNrm=(V3*)malloc((size_t)maxVertCount*sizeof(V3)); gWs=(V3*)malloc((size_t)maxVertCount*sizeof(V3)); if(!gSv||!gOk||!gCs||!gNrm||!gWs){ free(gSv); free(gOk); free(gCs); free(gNrm); free(gWs); gSv=NULL; gOk=NULL; gCs=NULL; gNrm=NULL; gWs=NULL; maxVertCount=0; } }\n";
                                mc << "  NB_KOS_InitInput();\n";
                                ...
                                mc << "      SV* sv = gSv; uint8_t* ok = gOk; V3* cs = gCs; V3* nrm = gNrm; V3* ws = gWs;\n";
                                ...
                                mc << "      }\n";
                                mc << "      /* Accumulate camera-space normals using cached weld groups + cached buffer */\n";
                                ...
                                mc << "        for (int t=0;t<rm->kTriCount;++t){ int ia=tris[t*3+0], ib=tris[t*3+1], ic=tris[t*3+2]; if(ia<0||ib<0||ic<0||ia>=rm->kVertCount||ib>=rm->kVertCount||ic>=rm->kVertCount) continue; V3 a=ws[ia], b=ws[ib], c=ws[ic]; float e1x=b.x-a.x,e1y=b.y-a.y,e1z=b.z-a.z,e2x=c.x-a.x,e2y=c.y-a.y,e2z=c.z-a.z; float nx=-(e1y*e2z-e1z*e2y), ny=-(e1z*e2x-e1x*e2z), nz=-(e1x*e2y-e1y*e2x); int ga=rm->weldGroups[ia],gb=rm->weldGroups[ib],gc=rm->weldGroups[ic]; gn[ga].x+=nx;gn[ga].y+=ny;gn[ga].z+=nz; gn[gb].x+=nx;gn[gb].y+=ny;gn[gb].z+=nz; gn[gc].x+=nx;gn[gc].y+=ny;gn[gc].z+=nz; }\n";
                                mc << "        for (int g=0;g<rm->nWeldGroups;++g) gn[g]=norm3(gn[g]);\n";
                                mc << "        /* Transform world-space normals to camera space */\n";
                                mc << "        for (int g=0;g<rm->nWeldGroups;++g){ V3 n=gn[g]; gn[g]=(V3){dot3(n,gBR),dot3(n,gBU),dot3(n,gBF)}; }\n";
                                ...
                                mc << "          float nC=dot3(nc,sLight); if(nC<0.0f)nC=0.0f; float litC=sAmb + nC*sDifScale - ((nc.y<0.0f)?(-nc.y*sShQ):0.0f) + (1.0f-fabsf(nc.z))*0.12f;\n";
